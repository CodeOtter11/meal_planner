...
mongoose.connect(mongoUri, {
  ...
  .catch(err => {
    console.error([${new Date().toISOString()}] MongoDB Connection Error: ${err.message}, err.stack);
    process.exit(1);
  });
...
try {
  ...
  console.error([${new Date().toISOString()}] Error loading auth routes: ${err.message}, err.stack);
  process.exit(1);
}
